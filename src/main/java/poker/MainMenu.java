...
public class MainMenu {
	...
	private void hostServerView() {
		...
		JLabel tables = generateLabel("Number of tables", 0, 2);
		gamePanel.add(tables, gridBagConstraints);

		SpinnerNumberModel tableModel = generateSpinner(5, 1, 1, 1, 1, 2);
		final JSpinner numberOfTables = new JSpinner(tableModel);
		gamePanel.add(numberOfTables, gridBagConstraints);

		JLabel cash = generateLabel("Starting amount of money", 0, 3);
		gamePanel.add(cash, gridBagConstraints);

		final JSpinner moneyAmount = new JSpinner();
		SpinnerNumberModel moneyModel = generateSpinner(500, 200, 20, 200, 1, 3);
		moneyAmount.setModel(moneyModel);
		;
		gamePanel.add(moneyAmount, gridBagConstraints);

		JLabel smallBlind = generateLabel("Small blind", 0, 4);
		...
		JLabel bigBlind = generateLabel("Big blind", 0, 5);
		...
		final JLabel raiseAmountLabel = generateLabel("Max raise amount", 0, 7);
		...
		SpinnerNumberModel raiseAmountModel = generateSpinner(100, 10, 10, 50, 1, 7);
		raiseAmount.setModel(raiseAmountModel);
		raiseAmount.setVisible(false);
		gamePanel.add(raiseAmount, gridBagConstraints);

		final JLabel raiseTimeLabel = generateLabel("Max raise times", 0, 8);
		...
		SpinnerNumberModel raiseTimeAmountModel = generateSpinner(5, 1, 1, 2, 1, 8);
		...
		JLabel rules = generateLabel("Rules", 0, 6);
		...
		JLabel name = generateLabel("Name", 0, 9);
		gamePanel.add(name, gridBagConstraints);
		...
		JLabel port = generateLabel("Port", 0, 10);
		gamePanel.add(port, gridBagConstraints);
		...
		createGame.addMouseListener(new MouseAdapter() {
			public void mousePressed(MouseEvent e) {
				gameScreen.hostServer((int) numberOfPlayers.getValue(), (int) numberOfTables.getValue(),
						(int) moneyAmount.getValue(), (int) portNumber.getValue(), (int) smallBlindAmount.getValue(),
						(int) bigBlindAmount.getValue(), (String) chooseRules.getSelectedItem(), nameText.getText(),
						(int) raiseAmount.getValue(), (int) raiseTimeAmount.getValue());
			}
		});
		gridBagConstraints.fill = GridBagConstraints.HORIZONTAL;
		gridBagConstraints.anchor = GridBagConstraints.EAST;
