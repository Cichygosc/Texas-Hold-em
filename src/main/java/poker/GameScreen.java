...
public class GameScreen {
	...
	public void hostServer(int players, int tables, int money, int port, int smallBlind, int bigBlind, String rules,
			String name, int raiseAmount, int raiseTimes) {
		try {
	...
	public void addBot(String name, int seat, int table) {
		try {
			gameModel.addBot(name, seat, table);
			gameModel.addTakenSeat(seat, name);

		} catch (IOException e) {
			messageLabel.setText("Couldnt add bot");
		}
	}
